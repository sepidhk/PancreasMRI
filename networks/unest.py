...
class UNesT(nn.Module):
    ...
    def __init__(
        ...
        self.decoder5 = UNesTBlock(
            ...
            stride=1,
            kernel_size=3,
            # I change the upsample kernel size for the first layer decoder
            # to 1 so that it will match 3,3,3 for encoder and decoder
            # upsample_kernel_size=2,
    ...
    def forward(self, x_in):
        ...
        enc4 = x4 # 2, 512, 6, 6, 6 (1, 512, 3)
        # print('enc4 shape: {}'.format(enc4.shape))

        # enc4 = self.encoder11(enc4)
        # print('new enc4 shape: {}'.format(enc4.shape))

        dec4 = x # 2, 512, 6, 6, 6 
        # print('dec4 shape: {}'.format(dec4.shape))

        dec4 = self.encoder10(dec4) # 2, 1024, 3, 3, 3 (1,1024,3)
        # print('new dec4 shape: {}'.format(dec4.shape))

        dec3 = self.decoder5(dec4, enc4) # 2, 512, 6, 6, 6 (1,512,3)
        # print('dec3 shape: {}'.format(dec3.shape))

        dec2 = self.decoder4(dec3, enc3) # 2, 256, 12, 12, 12 (1,256, 6)
